			...
			if (continue[i] == FALSE) {
				...
				if (i-start == 4){
				   lines(data[start:(i-1),])
					...
					polygon(data[start:(i-1),], destiny=dens[k], col=color[k], border="black")
				}
				start <- i+1
			}
		}
...
# Main function
       ...
       for (i in seq(along=code.list)) {
	      ...
	      code.lsit <- c(code.list, -code.list[i])
	      destiny <- c(destiny, destiny[i])
       ...
       lat <- ifelse(lat == 0, 0, lat-mlat+1)
       map0(cbind(as.integer(lon), as.integer(lat)), destiny, color )
}
